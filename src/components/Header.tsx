      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            <div className="px-2 sm:px-2.5 h-8 sm:h-9 rounded-lg bg-brand-orange flex items-center justify-center text-white font-black text-sm sm:text-base tracking-tight flex-shrink-0">
              AB
            </div>
            <span className="font-display font-bold text-lg sm:text-xl tracking-wide transition-colors duration-300 flex-shrink-0">
              <span className="text-[#FF6B35] font-extrabold">AB</span>
              <span className="text-slate-900 dark:text-white ml-1 font-semibold hidden sm:inline">Consulting</span>
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            ...
            <button 
              ...
              className="px-3 py-2 sm:px-5 sm:py-2.5 bg-brand-orange hover:bg-brand-orange-hover text-white text-[10px] sm:text-xs font-semibold rounded-full uppercase tracking-wider shadow-md hover:shadow-lg transition-all flex items-center gap-1.5 sm:gap-2 cursor-pointer flex-shrink-0"
            >
              <Database className="w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0" />
              <span className="hidden sm:inline">
                {activeUser 
                  ? `${activeUser.name.length > 10 ? activeUser.name.slice(0, 10) + '...' : activeUser.name}'s Portal` 
                  : 'Client Portal'}
              </span>
